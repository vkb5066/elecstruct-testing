...
def QR(n, A, giveVecs=False, smallTol=1e-8, itrLim=30):
    ...
    for i in range(n-1, 0, -1):
        ...
        for j in range(0, i):
            if(giveVecs):
                A[IndexTransform(n, j, i)] = A[IndexTransform(n, i, j)] / h

            ###A dot U
            g = 0 + 0j
            for k in range(0, j+1):
                g += conj(A[IndexTransform(n, j, k)]) * A[IndexTransform(n, i, k)]
            for k in range(j+1, i):
                g += A[IndexTransform(n, k, j)] * A[IndexTransform(n, i, k)]

            ###P
            t[j] = g/h
            f0 += conj(t[j])*A[IndexTransform(n, i, j)]

        hhparam = 0.5*f0/h

        ###reduce A, get Q
        for j in range(0, i):
            f0 = A[IndexTransform(n, i, j)]
            g = t[j] - hhparam*f0
            t[j] = g

            for k in range(0, j + 1):
                A[IndexTransform(n, j, k)] -= conj(f0)*t[k] + conj(g)*A[IndexTransform(n, i, k)]
        ...
        for i in range(0, n):
            if(abs(A[IndexTransform(n, i, i)]) > smallTol):
                for j in range(0, n):
                    g = 0. + 0j
                    for k in range(0, i):
                        g += conj(A[IndexTransform(n, k, i)])*B[IndexTransform(n, j, k)]
                    for k in range(0, i):
                        B[IndexTransform(n, j, k)] -= g*A[IndexTransform(n, i, k)]
