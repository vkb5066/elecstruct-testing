...
def QR(n, A, giveVecs=False, smallTol=1e-8, itrLim=30):
    ...
    def ssafehypot(a): ##aux function for (a^2 + 1)^(1/2) w/o under/overflow
        absa = abs(a)
        if(absa > 1.0):
            return absa*(1. + (1.0/absa)**(2))**(1/2)
        return (1. + (absa)**(2))**(1/2)
...
A = [[1.0 + 0.0j, 1.5 - 0.2j, 1.9 - 1.6j],
     [1.5 + 0.2j, 2.0 + 0.0,  4.7 + 2.8j],
     [1.9 + 1.6j, 4.7 - 2.8j, 3.0 + 0.0j]]
A, b = array(A), array(b)
...
vecs = transpose(vecs) ##usually vecs are returned as a matrix where the ith row of M is -> the ith eigenval
##                       but for checking the correctness of my QR method, they need to be columns
#print(round(v, 2), "\n\n", round(vecs, 2))
