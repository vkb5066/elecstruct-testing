...
def DavidFFT(n, mills, gs, kpt, vrgrid, gridsizes, nEigs, maxBasisSize=None, v0m=0, v0n=0,
    ...
    while(True):
        ...
        for i in range(0, currBasisSize): ##loop over bands
            psigridmain.fill(0.0 + 0.0j) ##memset -> 0
            ##Get coeffs of psi from V onto the real space grid
            psigridmain = hfft.GetPsig(npw=n, mills=mills, coeffs=V[i*n:(i+1)*n], sizes=gridsizes,
                                       psigrid=psigridmain)
            psigridmain = ifftn(x=psigridmain, s=(gridsizes[0], gridsizes[1], gridsizes[2]),
                                overwrite_x=False) * gridsizes[0]*gridsizes[1]*gridsizes[2]
            ##Compute the action psi @ A*
            ##TODO: V(r) should (i think) be pure real ... look into this
            for hi in range(0, gridsizes[0]):
                for ki in range(0, gridsizes[1]):
                    for li in range(0, gridsizes[2]):
                        psigridmain[hi][ki][li] = psigridmain[hi][ki][li] * vrgrid[hi][ki][li]
            ##Transform the grid back into W - we need V, so don't overwrite
            psigridmain = fftn(x=psigridmain, s=(gridsizes[0], gridsizes[1], gridsizes[2]),
                               overwrite_x=False) / (gridsizes[0]*gridsizes[1]*gridsizes[2])
            W[i*n:(i+1)*n] = hfft.GetPsiv(npw=n, mills=mills, coeffs=W[i*n:(i+1)*n],
                                          psigrid=psigridmain, sizes=gridsizes)
...
#Same function as above, but uses numby's convolution
#vrgrid should be sent as vggrid in this case, but for a smart implementation, you'd keep V(G) in real space
# ... the only reason we dont do that here is bc scipy can't handle that kind of complexity
from scipy.signal import fftconvolve
def DavidCnv(n, mills, gs, kpt, vggrid, gridsizes, nEigs, maxBasisSize=None, v0m=0, v0n=0,
             V0=None, eTol=1e-3):
    if(maxBasisSize == None):
        maxBasisSize = min(int(6.25*nEigs), n-1) ##emperically determined "good enough" max basis size
                                                 ##prob. will not be the same for c code since this isn't
    ...
    while(True):
        ...
        for i in range(0, currBasisSize): ##loop over bands
            ...
            psigridmain = fftconvolve(in1=psigridmain, in2=vggrid, mode="same", axes=None)
            #transform the grid back into W - we need V, so don't overwrite
            W[i*n:(i+1)*n] = hfft.GetPsiv(npw=n, mills=mills, coeffs=W[i*n:(i+1)*n],
                                          psigrid=psigridmain, sizes=gridsizes)
