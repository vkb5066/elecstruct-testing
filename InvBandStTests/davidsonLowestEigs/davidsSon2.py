...
def David(n, A, nEigs, maxBasisSize=None, V0exists=False, V0=None, eTol=1e-3):
    ...
    if(V0exists): ## ... but a better choice would be eigenvectors of a k-point w/ similar momentum
        ...
        V0 = empty(shape=(0,0), dtype=complex) ##just to make the algo work nicer

    for i in range(min(maxBasisSize, V0.shape[0]), maxBasisSize):
        norm = 0. ##strictly real
        for j in range(min(n, V0.shape[1]), n):
            V[i][j] = random() + random()*1j
            norm += conj(V[i][j])*V[i][j]
        ##Normalize
        norm = 1./sqrt(norm)
        for j in range(0, n):
            V[i][j] = V[i][j]*norm
